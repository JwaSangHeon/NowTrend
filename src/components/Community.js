...
//     UserData.filter((keyword) => keyword.classification === "community").sort(
...
function Community() {
  ...
  const [datas, setDatas] = useState(
    UserData.filter((keyword) => keyword.classification === "community")
  ...
  const options = {
    responsive: true,
    aspectRatio: 1,
    maintainAspectRatio: datas.length > 5 ? true : false,
    plugins: {
      legend: {
        labels: {
          color: "white",
